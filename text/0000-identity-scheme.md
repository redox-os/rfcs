...
To fix this, this proposal introduces a kernel-level identity scheme to replace these integer IDs with unforgeable, capability-based file descriptors called `identity_fd`s. An `identity_fd` serves as a verifiable handle to a user's identity that can be securely propagated through any intermediary, ensuring the final recipient can always perform accurate, spoof-proof permission checks against the true originator of a request.
...
## Privilege Management (procmgr's `setresugid`, and `sudo`)

- `setresugid`: This operation will be managed by the identity scheme but orchestrated by procmgr. To change a process's identity, the process sends a request to procmgr. procmgr then performs the following steps:
    1. It takes the `identity_fd` from one of the process's threads and prepares a buffer containing the new UID/GID information.
    2. It calls `dup(current_identity_fd, &resugid_buf)`. This single call requests the identity scheme to create a new identity resource.
    3. The identity scheme validates the request by checking if procmgr's own `identity_fd` possesses the `setresugid` execution capability. This highly privileged capability is granted to procmgr at boot time.
    4. If the request is valid, the identity scheme returns the `new_identity_fd` to procmgr.
    5. Finally, procmgr iterates through all threads of the target process, performing fd passing on each thread's context handle (`ctxt_fd`) to atomically replace their old identity handle with the `new_identity_fd`.
...
- User/Group Management: Where should the logic for creating, deleting, and modifying users/groups reside? Should procmgr be given capability fds to manage this, or should a new dedicated userspace scheme be created?
